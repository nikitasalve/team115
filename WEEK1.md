...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/FTP115 `
    * `cd FTP115`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\FTP115
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE FTP115;` and click the lightning button
  * `CREATE USER 'FTP115'@'localhost' IDENTIFIED BY 'FTP115';`
  * `GRANT ALL ON FTP115.* TO 'FTP115'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/FTP115
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.FTP115.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/FTP115-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/FTP115.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/FTP115/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/FTP115/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/FTP115-0.0.1-SNAPSHOT.war ./FTP115.war`
   * `cd dist`
   * `jar -uvf ../FTP115.war *`
   * `cd ..`
   * `jar -tvf FTP115.war`
   * `cp FTP115.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/FTP115.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/FTP115/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "FTP115"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "FTP115 Sprint 1" as label 
